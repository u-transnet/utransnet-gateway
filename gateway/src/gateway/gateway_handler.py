...
from transnet.account import Account as TRNTAccount
from transnet.amount import Amount as TRNTAmount
from transnet.asset import Asset as TRNTAsset
from transnetbase import operations as TRNToperations
from transnet.storage import configStorage as TRNTconfig
...
class BaseGatewayHandler(AccountListenerBaseHandler):
    def __init__(self,
                 ...
                 base_asset, assets_mapping=None):
...
class BitsharesGatewayHandler(BaseGatewayHandler):
    def issue_assets(self, blockchain_instance, amount, issuer, to, target_asset):
        ...
        operation = BTSoperations.Asset_issue(**{
            "fee": {"amount": 0, "asset_id": BTSAsset(self.base_asset)['id']},
            "issuer": issuer["id"],
...
class TransnetGatewayHandler(BaseGatewayHandler):
    def issue_assets(self, blockchain_instance, amount, issuer, to, target_asset):
        if not 'default_account' in TRNTconfig:
            raise ValueError("You need to provide an account")

        issuer = TRNTAccount(issuer)
        to = TRNTAccount(to)
        target_asset = TRNTAsset(target_asset)

        operation = TRNToperations.Asset_issue(**{
            "fee": {"amount": 0, "asset_id": TRNTAsset(self.base_asset)['id']},
            "issuer": issuer["id"],
            "issue_to_account": to["id"],
            "asset_to_issue": {
                "amount": amount,
        ...
        blockchain_instance.finalizeOp(operation, TRNTconfig['default_account'], 'active')

    def burn_assets(self, amount, asset, account):
        self.blockchain_api_external.reserve(TRNTAmount(
            amount=amount,
            asset=asset,
